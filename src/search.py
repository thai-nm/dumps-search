...
class SearchEngine:
    ...
    def __init__(self, max_results: int = 10, retry_attempts: int = 3, retry_delay: float = 1.0):
    ...
    def search_question(self, keyword: str, title: str, url_substring: str) -> Optional[str]:
        ...
        try:
            ...
            if not results:
                self.logger.warning(f"No search results found for query: {search_query}")
                return None
    ...
    def _perform_search(self, query: str) -> List[dict]:
        ...
        raise Exception(f"All {self.retry_attempts} search attempts failed. Last error: {str(last_exception)}")
    
    def get_first_valid_url(self, results: List[dict], url_substring: str) -> Optional[str]:
    ...
    def validate_url(self, url: str, url_substring: str) -> bool:
        ...
        try:
            ...
            if url_substring.lower() in url.lower():
                ...
                self.logger.debug(f"URL rejected (missing substring '{url_substring}'): {url}")
                return False
